...
namespace rtl
{
  ...
  void destroyT(SEXP p)
  {
    if( TYPEOF(p) == EXTPTRSXP )
      delete static_cast<T*>(R_ExternalPtrAddr(p));
  }
  ...
  inline SEXP createT(SEXP s4)
  {
    ...
    if (obj->validate(s4))
    {
      ...
      R_do_slot_assign(s4, Rf_install(SLOT_CLASS_NAME), pt.add(tools::newVal(T::class_name)));
    }
    else
    {
      ATLASSERT(0);
  ...
  T* getCObject(SEXP s4)
  {
    ...
    return static_cast<T*>(R_ExternalPtrAddr(ptr));
  }
