...
namespace tools
{
  ...
  {
    Rf_setAttrib(obj, R_NamesSymbol, newVal(names));
    return obj;
  }
  ...
  public:
    ...
    /*template<typename T>
    //typename std::enable_if<!std::is_same<T, SEXP>::value, SEXP>::type 
    SEXP push_back(const T &val, const LPCWSTR name = NULL)
    {
      //if (std::is_same<T, SEXP>::value)
      //  return push_back(val, toUtf8(name).c_str());
      return _push_back(tools::newVal(val), toUtf8(name).c_str());
    }*/
    template<typename T>
  ...
  inline SEXP newVal(SAFEARRAY& psa)
  {
    ...
    ATLASSERT(lUBound - lLBound + 1 > 0); //???
    size_t size = (size_t)(lUBound - lLBound + 1);
...
inline SEXP showError(const char* strErrDefaultUTF8 = "unknown error")
{
  if (bCheckCOM)
  {
    auto str = arcobject::getLastComError();
    if (!str.empty())
      return error_Ret(str.c_str());
  }
